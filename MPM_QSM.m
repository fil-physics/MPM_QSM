...
% Image B
imB = nifti([pn 'imB.nii']);

A2B = imA.mat\imB.mat;

dataOut = zeros(imA.dat.dim);
for slice = 1 : imA.dat.dim(3)
    dataOut(:,:,slice) = spm_slice_vol(spm_vol(imB.dat.fname),A2B*spm_matrix([0 0 slice]), imB.dat.dim(1:2), -7);
end
