...
in_root_dir = '/media/barbara/hdd2/DATA/FIL/7T/M700153_analysis/MORSE_Opt_phase/' ;
% the data will be saved:
out_root_dir = '/media/barbara/hdd2/DATA/FIL/7T/M700153_analysis/MORSE_Opt_phase/SEPIA/';
for run = 1
    
    switch run
        
        case 1 %pdw
            mag_dir = 'pdw_mfc_3dflash_v1k_RR_0054' ; % folder with magnitude niftis
            ph_dir = 'pdw_mfc_3dflash_v1k_RR_0055' ; % folder with phase inftis
            TEs = [2.2 4.58 6.96 9.34 11.72 14.1] ; % echo time in ms
            output_dir = 'pdw_RR_54_55_rot' ; % output directory for a specific submeasurement from MPM
            mag_file = 's2021-05-24_14-26-153624-00001-01728-6.nii' ; % magnitude reference nifti file for ROMEO unwrapping and masking
            
        case 2 % t1w
            mag_dir = 't1w_mfc_3dflash_v1k_RR_0056' ;
            ph_dir = 't1w_mfc_3dflash_v1k_RR_0057' ;
        ...
        case 3 % mtw
            mag_dir = 'mtw_mfc_3dflash_v1k_180deg_RR_0058' ;
            ph_dir = 'mtw_mfc_3dflash_v1k_180deg_RR_0059' ;
...
%         qmap_bin = imfill(qmap_bin,8,'holes') ; % maybe add as an user option?
        ...
        save_untouch_nii(qmask_nii, qmask_file);
        reslice_nii('mask.nii', 'mask_rot.nii', ph_1tp.hdr.dime.pixdim(2:4))
    end
   ...
   % creating inverse affine matrix for rotation back to the initial image
   % space
% % %     M_aff(1,:) = ph_1tp.hdr.hist.srow_x ;
% % %     M_aff(2,:) = ph_1tp.hdr.hist.srow_y ;
% % %     M_aff(3,:) = ph_1tp.hdr.hist.srow_z ;
% % %     M_aff(4,:) = [ 0 0 0 1] ;
% % %     
% % %     M_inv = eye(4,4) + -1*(1-eye(4,4)) ;
% % %     M_aff_inv = M_aff.*M_inv ;
    
% % %     QSM.hdr.hist.srow_x = M_aff_inv(1,:) ;
% % %     QSM.hdr.hist.srow_y = M_aff_inv(2,:) ;
% % %     QSM.hdr.hist.srow_z = M_aff_inv(3,:) ;
    QSM_invrot = flipdim(permute(QSM.img, [2 3 1]),1);
M_aff(1,:) = [0.8660    0.5000         0         0] ;
    M_aff(2,:) = [-0.5000    0.8660         0         0];
    M_aff(3,:) = [     0         0    1.0000         0];
    M_aff(4,:) = [0         0         0    1.0000];
[QSM_invrot new_M] = affine(QSM_invrot, M_aff, ph_1tp.hdr.dime.pixdim(2:4));
% % %     QSM.hdr.hist.srow_x = ph_1tp.hdr.hist.srow_x ;
% % %     QSM.hdr.hist.srow_y = ph_1tp.hdr.hist.srow_y ;
% % %     QSM.hdr.hist.srow_z = ph_1tp.hdr.hist.srow_z ;
    QSM_invrot_file = sprintf('sepia_%s_%s_QSM_invrot.nii.gz', algorParam.bfr.method, algorParam.qsm.method) ;
    save_untouch_nii(make_nii(QSM_invrot), QSM_invrot_file)
    reslice_inv_nii(QSM_invrot_file, 'test.nii', ph_1tp.hdr.dime.pixdim(2:4))
