...
para.in_root_dir = '/media/barbara/hdd2/DATA/FIL/7T/20210623.M700198_FIL_analysis' ;
para.out_root_dir = '/media/barbara/hdd2/DATA/FIL/7T/20210623.M700198_FIL_analysis/SEPIA/BART_scan1'; 
...
for run = 1:3
    ...
    switch run
        case 1 %PDw
            ...
            para.ph_dir = 'pdw_mfc_3dflash_v1k_RR_0052' ; % folder with phase inftis
            para.TEs = [2.2 4.58 6.96 9.34 11.72 14.1] ; % echo time in ms
            para.output_dir = 'pdw_52_53' ; % output directory for a specific submeasurement from MPM
            
        case 2 % T1w
            para.mag_dir = 't1w_mfc_3dflash_v1k_RR_0050' ;
            para.ph_dir = 't1w_mfc_3dflash_v1k_RR_0051' ;
            para.TEs = [2.3 4.68 7.06 9.44 11.82 14.2] ;
            para.output_dir = 't1w_50_51' ;
            
        case 3 % MTw
            para.mag_dir = 'mtw_mfc_3dflash_v1k_180deg_RR_0054' ;
            para.ph_dir = 'mtw_mfc_3dflash_v1k_180deg_RR_0055' ;
            para.TEs = [2.2 4.58 6.96 9.34] ; % echo time in ms
            para.output_dir = 'mtw_54_55' ;
 
    end
    
    [QSM_all(:,:,:,run), QSM_all_invrot(:,:,:,run)] = MPM_QSM(para) ;
...
QSM_V.fname = fullfile(out_root_dir,'QSM_pdw_t1w_mean.nii') ;
spm_write_vol(QSM_V, QSM_pdw_t1w_mean)
QSM_V.fname = fullfile(out_root_dir,'QSM_all_mean.nii') ;
spm_write_vol(QSM_V, QSM_all_mean)

QSMinvrot_V.fname = fullfile(out_root_dir,'QSM_pdw_t1w_invrot_mean.nii') ;
spm_write_vol(QSMinvrot_V, QSM_pdw_t1w_invrot_mean)
QSMinvrot_V.fname = fullfile(out_root_dir,'QSM_all_invrot_mean.nii') ;
